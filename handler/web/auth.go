...
func (a *authWeb) Login(w http.ResponseWriter, r *http.Request) {
	...
	viewLogin := path.Join("views", "auth", "login.html")
	
	tmp, err := template.ParseFS(a.embed, viewLogin, header)
	if err != nil {
		log.Println(err)
...
func (a *authWeb) Register(w http.ResponseWriter, r *http.Request) {
	// TODO: answer here
	header := path.Join("views", "general", "header.html")
	viewRegister := path.Join("views", "auth", "register.html")
	
	tmp, err := template.ParseFS(a.embed, viewRegister, header)
	if err != nil {
		log.Println(err)
		w.WriteHeader(http.StatusInternalServerError)
		return
